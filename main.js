...
window.addEventListener('DOMContentLoaded', () => {
  // Animation timeout
  setTimeout(() => {
    // Adding active class to logo parts with a delay
    ...
    // Removing active class and adding fade class to logo parts with a delay
    setTimeout(() => {
      logoSpan.forEach((span, index) => {
        setTimeout(() => {
    ...
    // Hiding the intro section after a delay
    setTimeout(() => {
      intro.style.top = '-100vh';
    }, 5000);
  }, 0); // Set timeout delay to 0 for immediate execution
});
